...
exec /usr/bin/autossh -M 0 -o "ServerAliveInterval 30" -o "ServerAliveCountMax 3" -o "ExitOnForwardFailure yes" -o "StrictHostKeyChecking no" -o "UserKnownHostsFile /dev/null" -N -R 0.0.0.0:${UI_PORT}:localhost:8002  -R 0.0.0.0:${TUNNEL_PORT}:localhost:22 -i /root/.ssh/CREM3-API-03.pem ubuntu@${REMOTE_HOST}