...
async def lifespan(inner_app: FastAPI):
    global manager
    logger.info("Starting up actuator manager...")
    try:
        manager = ActuatorManager()
        success1 = manager.add_actuator(1, node_id=0x1B)
        success2 = manager.add_actuator(2, node_id=0x1C)
        
        if not (success1 and success2):
            logger.error("Failed to initialize all actuators")
            raise RuntimeError("Actuator initialization failed")
        yield
...
<html>
<head>
    <title>Actuator Control System</title>
    <style>
        body { 
